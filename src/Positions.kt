...
data class Pos(val x: Long, val y: Long) {
    ...
    fun isBetween(other1: Pos, other2: Pos): Boolean {
        return sameX(listOf(other1, other2)) && this.yIsBetween(other1, other2) || sameY(
            listOf(
                other1,
                other2
            )
        ) && this.xIsBetween(other1, other2)
    }

    fun sameX(others: List<Pos>): Boolean = others.all { it.x == this.x }
    fun sameX(other1: Pos, other2: Pos): Boolean = sameX(listOf(other1, other2))
    fun sameY(others: List<Pos>): Boolean = others.all { it.y == this.y }
    fun sameY(other1: Pos, other2: Pos): Boolean = sameY(listOf(other1, other2))
